...
from similarityValue import simInverseFrequency
from similarityValue import corInverseFrequency
import math
...
def main():
    ...
    movieTraining=dataTraining.transpose()

    movieFreq={}
    for i in range(0,1000):
        movieFreq[i+1]=200-Counter(movieTraining[i])[0]
    ...
    for i in range(0+userOff,100+userOff):
        for k in range(1,201):
            sim=corInverseFrequency(testDict[i],trainDict[k],movieFreq)
    ...
    for i in range(0,len(testPredic)):
        ...
        for k in range(1,201):
            ...
            if sim!=0:
                if sim>0:
                    sim=math.pow(sim,1.5)
                else:
                    sim=math.pow(-1*sim,1.5)*-1
