...
def main():
    ...
    for i in range(0,len(testPredic)):
        ...
        for k in range(1,201):
            ...
            if sim!=0:
                if sim>0:
                    sim=math.pow(sim,1.2)
                else:
                    sim=math.pow(-1*sim,1.2)*-1
                summ= summ+(sim*(trainDict[k][activeMID-1]-meanVoteTrainDict[k])/np.std(trainDict[k]))
                normFactor=normFactor+abs(sim)
        predictedRating=1
        if(normFactor!=0):
            predictedRating=meanVoteTestDict[activeUID]+(np.std(testDict[activeUID])*(summ/normFactor))
